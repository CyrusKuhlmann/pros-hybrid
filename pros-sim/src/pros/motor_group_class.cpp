...
std::int32_t MotorGroup::move_velocity(const std::int32_t velocity) const {
  ...
  for (auto p : _ports)
    sim::SimClient::instance().send_motor_move(ap(p), sg(p) * v);
  return 1;
}
...
std::int32_t MotorGroup::brake() const {
  for (auto p : _ports) sim::SimClient::instance().send_motor_move(ap(p), 0);
  return 1;
}
...
std::int32_t MotorGroup::get_voltage(const std::uint8_t index) const {
  if (index >= _ports.size()) return 0;
  auto st = sim::SimClient::instance().get_motor_state(ap(_ports[index]));
  return sg(_ports[index]) * static_cast<int32_t>(st.voltage * 12000.0 / 127.0);
}
...
std::vector<double> MotorGroup::get_target_position_all()
    const {MG_ALL(double, get_target_position)} std::vector<
        std::int32_t> MotorGroup::get_target_velocity_all()
        const {MG_ALL(std::int32_t, get_target_velocity)} std::vector<
            double> MotorGroup::get_actual_velocity_all()
            const {MG_ALL(double, get_actual_velocity)} std::vector<
                std::int32_t> MotorGroup::get_current_draw_all()
                const {MG_ALL(std::int32_t, get_current_draw)} std::vector<
                    std::int32_t> MotorGroup::get_direction_all()
                    const {MG_ALL(std::int32_t, get_direction)} std::vector<
                        double> MotorGroup::get_efficiency_all() const {
...
std::vector<double> MotorGroup::get_position_all() const {
    MG_ALL(double,
           get_position)} std::vector<double> MotorGroup::get_power_all()
    const {MG_ALL(double, get_power)} std::vector<std::int32_t> MotorGroup::
...
std::vector<double> MotorGroup::get_temperature_all() const {MG_ALL(
    double,
    get_temperature)} std::vector<double> MotorGroup::get_torque_all() const {
    MG_ALL(double,
           get_torque)} std::vector<std::int32_t> MotorGroup::get_voltage_all()
    const {MG_ALL(std::int32_t, get_voltage)} std::vector<
        std::int32_t> MotorGroup::is_over_current_all()
        const {MG_ALL(std::int32_t, is_over_current)} std::vector<
            std::int32_t> MotorGroup::is_over_temp_all() const {
...
std::int32_t MotorGroup::set_brake_mode(const MotorBrake mode,
  ...
  if (index < _ports.size())
    sim::SimClient::instance().send_motor_set_brake_mode(ap(_ports[index]),
                                                         bm_str(mode));
  return 1;
}
std::int32_t MotorGroup::set_brake_mode(const pros::motor_brake_mode_e_t mode,
  ...
  if (index < _ports.size())
    sim::SimClient::instance().send_motor_set_brake_mode(ap(_ports[index]), m);
  return 1;
}
std::int32_t MotorGroup::set_brake_mode_all(const MotorBrake mode) const {
  for (auto p : _ports)
    sim::SimClient::instance().send_motor_set_brake_mode(ap(p), bm_str(mode));
  return 1;
}
