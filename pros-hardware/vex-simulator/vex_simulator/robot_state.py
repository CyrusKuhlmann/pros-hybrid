...
class RobotState:
    ...
    def handle_command(self, cmd: CommandMessage) -> None:
        ...
        if cmd.type == MessageType.SET_MOTOR_VOLTAGE:
        ...
        elif cmd.type == MessageType.SET_MOTOR_VELOCITY:
            motor = self.get_or_create_motor(cmd.port)
            velocity = cmd.params.get("velocity", 0)
            motor.set_velocity(velocity)
            print(f"Motor {cmd.port}: velocity={velocity}")

        elif cmd.type == MessageType.SET_MOTOR_ABSOLUTE:
            motor = self.get_or_create_motor(cmd.port)
            position = cmd.params.get("position", 0.0)
            velocity = cmd.params.get("velocity", 200)
            motor.move_absolute(position, velocity)
            print(f"Motor {cmd.port}: move_absolute pos={position} vel={velocity}")

        elif cmd.type == MessageType.SET_MOTOR_RELATIVE:
            motor = self.get_or_create_motor(cmd.port)
            position = cmd.params.get("position", 0.0)
            velocity = cmd.params.get("velocity", 200)
            motor.move_relative(position, velocity)
            print(f"Motor {cmd.port}: move_relative pos={position} vel={velocity}")

        elif cmd.type == MessageType.MOTOR_BRAKE:
            motor = self.get_or_create_motor(cmd.port)
            motor.brake()
            print(f"Motor {cmd.port}: brake")
            ...
            motor = self.get_or_create_motor(cmd.port)
            motor.tare_position()
            print(f"Motor {cmd.port}: tare_position")
            ...
            print(f"Rotation {cmd.port}: set_position={position}")
